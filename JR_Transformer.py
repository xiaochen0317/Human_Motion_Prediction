...
class Attention(nn.Module):
    ...
    def forward(self, joint_feature, relation_feature, mask=None):
        ...
        R_qkv = self.R_qk(relation_feature).reshape(B, N, N, 2, H, HS).permute(3, 0, 4, 1, 2,
                                                                               5)  # [3, B, #heads, N, N, C//#heads]
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mlp_relation1 = Mlp(in_features=dim * 4, hidden_features=dim * 4, out_features=dim, act_layer=act_layer,
                                 drop=drop)
    ...
    def forward(self, joint_feature, relation_feature, mask=None):
        ...
        joint_feature = joint_feature + self.drop_path(
            self.attn(self.norm_attn1(joint_feature), self.norm_attn2(relation_feature), mask))
...
class JRTransformer(nn.Module):
    def __init__(self, N=2, J=13, in_joint_size=16 * 6, in_relation_size=18, feat_size=128, out_joint_size=30 * 3,
